...
resource "local_file" "ansibly_inventory" {
  content  = <<-XYZ

[webservers]
web-a ansible_host=${yandex_compute_instance.web_a.network_interface.0.ip_address} ansible_user=user
web-b ansible_host=${yandex_compute_instance.web_b.network_interface.0.ip_address} ansible_user=user

[bastion_host]
bastion ansible_host=${yandex_compute_instance.bastion.network_interface.0.nat_ip_address} ansible_user=user
...
resource "local_file" "ansibly_ssh" {
  content  = <<-XYZ
